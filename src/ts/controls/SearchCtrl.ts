...
} from "../util/Autocompleter";

export interface SearchControlOptions<T extends AutocompleteItem>
  extends AutocompleteSettings<T> {
...
export class SearchControl extends Control {
  ...
  onAdd(map: L.Map): HTMLElement {
    ...
    const bttn = (this._Bttn = createHtmlElement(
      "button",
      this._container,
      "search-button ctrl-icon"
    ));
  ...
  createSearchField() {
    const searchDom = (this.searchDom = createHtmlElement(
      "div",
      undefined,
      "search"
    ));
    ...
    const closeBttn = (this.closeBttn = createHtmlElement(
      "div",
      searchDom,
      "angels-up"
    ));
  ...
  clearClicked() {
    console.info("clearClicked not implemented");
  }
}
